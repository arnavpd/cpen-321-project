...
import { ProjectController } from '../../mocks/projectController.mock';
...
describe('Mocked: POST /api/projects', () => {
  ...
  test('Invitation Code Generation Fails - Returns 500 Internal Server Error', async () => {
    ...
    mockProjectModel.findByOwnerId.mockResolvedValue([]);
    mockProjectModel.generateInvitationCode.mockRejectedValue(new Error('Failed to generate code'));
    ...
    expect(mockProjectModel.generateInvitationCode).toHaveBeenCalled();
    // Should not attempt to create project since generation failed
